...
weitron/
├── apps/
│   ├── frontend/   React + Vite + Tailwind（見 apps/frontend/README.md）
│   └── backend/    FastAPI + SQLAlchemy（見 apps/backend/README.md）
├── packages/       未來共用程式碼（如共用型別）存放處，目前留空
├── pnpm-workspace.yaml
├── package.json    monorepo 根設定
├── CODEOWNERS      資料夾層級的 PR 負責人指派
└── exam_questions_data.json   題庫種子資料（給 apps/backend/app/seed.py 匯入用）
```

前後端各自獨立可運行、獨立部署，但共用同一個 git 歷史。若後端工程師只想拿到後端程式碼，可用：
...
這樣工作目錄只會出現 `apps/backend`，不會混到前端程式碼；`CODEOWNERS` 也已設定 `apps/frontend/` 與 `apps/backend/` 分別指派給對應負責人，PR 會自動請對的人 review。

---

## 1. 專案概述

**目的**：練習「中餐丙級技術士技能檢定」術科考試，呈現每題的考試流程、操作步驟、時間限制與評分重點。
